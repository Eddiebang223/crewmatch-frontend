...
const TrustBadge = () => (
  <div style={styles.trustBar}>
    <div style={styles.trustItem}><span style={styles.trustIcon}>✓</span><span>10,000+ Jobs Completed</span></div>
    <div style={styles.trustItem}><span style={styles.trustIcon}>⭐</span><span>4.9/5 Rating</span></div>
    <div style={styles.trustItem}><span style={styles.trustIcon}>🔒</span><span>Secure Payments</span></div>
    <div style={styles.trustItem}><span style={styles.trustIcon}>⚡</span><span>24hr Response Time</span></div>
  </div>
);
...
const Button = ({ children, onClick, variant = 'primary', size = 'medium', disabled, fullWidth }) => (
  <button onClick={onClick} disabled={disabled} style={{ ...styles.button, ...styles.buttonSizes[size], ...styles.buttonVariants[variant], ...(fullWidth && styles.fullWidth), ...(disabled && styles.buttonDisabled) }}>
    {children}
  </button>
);
  ...
  <div style={styles.hero}>
    <div style={styles.heroOverlay}>
      <div style={styles.heroContent}>
        ...
        <p style={styles.heroSubtitle}>CrewMatch connects General Contractors with vetted, reliable tradespeople. Post jobs, compare bids, and hire with confidence.</p>
    ...
    <div style={styles.section}>
      <div style={styles.container}>
        ...
        <div style={styles.featuresGrid}>
          <div style={styles.featureCard}><div style={styles.featureIcon}>🎯</div><h3>Smart Matching</h3><p>AI-powered matching connects you with qualified tradespeople based on skills, location, and availability.</p></div>
          <div style={styles.featureCard}><div style={styles.featureIcon}>💳</div><h3>Secure Payments</h3><p>Escrow protection ensures contractors get paid and GCs get quality work. Every transaction is insured.</p></div>
          <div style={styles.featureCard}><div style={styles.featureIcon}>⭐</div><h3>Verified Reviews</h3><p>Real reviews from verified projects help you make informed hiring decisions with confidence.</p></div>
          <div style={styles.featureCard}><div style={styles.featureIcon}>📱</div><h3>Mobile Ready</h3><p>Manage jobs and bids from anywhere with our responsive design and mobile-friendly interface.</p></div>
        </div>
      </div>
    </div>
    ...
    <div style={styles.statsSection}>
      <div style={styles.container}>
        <div style={styles.statsGrid}>
          <div style={styles.statCard}><span style={styles.statIcon}>✅</span><div><div style={styles.statValue}>10,000+</div><div style={styles.statLabel}>Jobs Completed</div></div></div>
          <div style={styles.statCard}><span style={styles.statIcon}>🏢</span><div><div style={styles.statValue}>500+</div><div style={styles.statLabel}>Trusted Companies</div></div></div>
          <div style={styles.statCard}><span style={styles.statIcon}>⚡</span><div><div style={styles.statValue}>48hrs</div><div style={styles.statLabel}>Avg. Time to Hire</div></div></div>
          <div style={styles.statCard}><span style={styles.statIcon}>⭐</span><div><div style={styles.statValue}>4.9/5</div><div style={styles.statLabel}>Customer Rating</div></div></div>
        </div>
      </div>
    </div>
    ...
    <div style={styles.authContainer}>
      ...
      <Card title="Sign In" variant="auth">
        <form onSubmit={handleSubmit}>
          ...
          <div style={styles.inputGroup}><label>Email Address</label><input type="email" placeholder="you@company.com" value={email} onChange={(e) => setEmail(e.target.value)} required style={styles.input} /></div>
          <div style={styles.inputGroup}><label>Password</label><input type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} required style={styles.input} /></div>
          <Button type="submit" disabled={loading} fullWidth size="large">{loading ? 'Signing in...' : 'Sign In'}</Button>
          <p style={styles.authFooter}>Don't have an account? <Link to="/register">Create an account</Link></p>
        </form>
      </Card>
    </div>
  );
  ...
  return (
    <div style={styles.authContainer}>
      ...
      <Card title="Create Account" variant="auth">
        <form onSubmit={handleSubmit}>
          {error && <div style={styles.errorMsg}>{error}</div>}
          <div style={styles.inputGroup}><label>Full Name</label><input type="text" name="name" placeholder="John Smith" value={formData.name} onChange={handleChange} required style={styles.input} /></div>
          <div style={styles.inputGroup}><label>Email Address</label><input type="email" name="email" placeholder="you@company.com" value={formData.email} onChange={handleChange} required style={styles.input} /></div>
          <div style={styles.inputGroup}><label>Password</label><input type="password" name="password" placeholder="Create a strong password" value={formData.password} onChange={handleChange} required style={styles.input} /></div>
          <div style={styles.inputGroup}><label>I am a...</label><select name="role" value={formData.role} onChange={handleChange} style={styles.select}><option value="CONTRACTOR">Trade Professional (Electrician, Plumber, etc.)</option><option value="GC">General Contractor</option></select></div>
          <div style={styles.inputGroup}><label>{formData.role === 'GC' ? 'Company Name' : 'Business Name'}</label><input type="text" name="companyName" placeholder={formData.role === 'GC' ? "Smith Construction" : "Smith Electrical"} value={formData.companyName} onChange={handleChange} style={styles.input} /></div>
          <Button type="submit" disabled={loading} fullWidth size="large">{loading ? 'Creating Account...' : 'Get Started'}</Button>
          <p style={styles.authFooter}>Already have an account? <Link to="/login">Sign in</Link></p>
        </form>
      </Card>
    </div>
  );
    ...
    <div style={styles.dashboard}>
      ...
      <div style={styles.dashboardContent}>
        <div style={styles.welcomeSection}>
          <div><h1>Welcome back, {user.name} 👋</h1><p>{user.role === 'GC' ? 'Post jobs and find qualified contractors instantly.' : 'Find jobs and grow your contracting business.'}</p></div>
          <div style={styles.quickStats}>
            <div style={styles.quickStat}><span>Active Jobs</span><strong>0</strong></div>
            <div style={styles.quickStat}><span>Completed</span><strong>0</strong></div>
            <div style={styles.quickStat}><span>Rating</span><strong>⭐ 0.0</strong></div>
          </div>
        </div>
        <div style={styles.grid}>
          {user.role === 'GC' ? (
            <>
              <Card title="Post a Job" icon="📋"><p>Create a new job posting and get bids from qualified contractors.</p><Button onClick={() => window.location.href = '/post-job'}>Post Job →</Button></Card>
              <Card title="My Jobs" icon="📊"><p>View all your active and completed jobs.</p><Button onClick={() => window.location.href = '/my-jobs'} variant="secondary">View Jobs →</Button></Card>
            </>
          ) : (
            <>
              <Card title="Find Work" icon="🔍"><p>Browse available jobs in your area and submit bids.</p><Button onClick={() => window.location.href = '/jobs'}>Find Jobs →</Button></Card>
              <Card title="My Bids" icon="📝"><p>Track your submitted bids and see their status.</p><Button onClick={() => window.location.href = '/my-bids'} variant="secondary">View Bids →</Button></Card>
            </>
          )}
        </div>
      </div>
...
const styles = {
  ...
  navLink: { color: colors.dark, textDecoration: 'none', fontWeight: '500' },
  registerBtn: { background: colors.primary, color: colors.white, padding: '0.6rem 1.2rem', borderRadius: '8px', textDecoration: 'none', fontWeight: '500' },
  ...
  logoutBtn: { background: 'transparent', color: colors.dark, border: `1px solid ${colors.lightGray}`, padding: '0.5rem 1rem', borderRadius: '8px', cursor: 'pointer' },
  ...
  primaryBtn: { background: colors.white, color: colors.primary, padding: '0.8rem 2rem', borderRadius: '8px', textDecoration: 'none', fontWeight: '600' },
  ...
  featureCard: { textAlign: 'center', padding: '2rem', borderRadius: '16px', background: colors.white, boxShadow: '0 4px 20px rgba(0,0,0,0.05)' },
  ...
  input: { width: '100%', padding: '0.75rem 1rem', border: `1px solid ${colors.lightGray}`, borderRadius: '8px', fontSize: '1rem' },
  ...
  card: { background: colors.white, borderRadius: '16px', padding: '2rem', boxShadow: '0 4px 20px rgba(0,0,0,0.05)' },
  ...
  pagePlaceholder: { minHeight: 'calc(100vh - 64px)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem', color: colors.gray, background: colors.lightGray, gap: '1rem' },
