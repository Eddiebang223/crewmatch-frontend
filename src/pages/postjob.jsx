    ...
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        <form onSubmit={handleSubmit}>
          <div style={styles.formGroup}><label>Job Title *</label><input type="text" name="title" value={formData.title} onChange={handleChange} required placeholder="e.g., Commercial Electrical Installation" style={styles.input} /></div>
          <div style={styles.formGroup}><label>Trade *</label><select name="trade" value={formData.trade} onChange={handleChange} required style={styles.select}>{trades.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
          <div style={styles.formGroup}><label>Description *</label><textarea name="description" value={formData.description} onChange={handleChange} required rows="4" placeholder="Describe the scope of work..." style={styles.textarea} /></div>
          <div style={styles.formGroup}><label>Location *</label><input type="text" name="location" value={formData.location} onChange={handleChange} required placeholder="City, State" style={styles.input} /></div>
          <div style={styles.row}><div style={styles.formGroup}><label>Start Date *</label><input type="date" name="startDate" value={formData.startDate} onChange={handleChange} required style={styles.input} /></div><div style={styles.formGroup}><label>End Date *</label><input type="date" name="endDate" value={formData.endDate} onChange={handleChange} required style={styles.input} /></div></div>
          <div style={styles.row}><div style={styles.formGroup}><label>Hours *</label><input type="number" name="hours" value={formData.hours} onChange={handleChange} required placeholder="Total hours" style={styles.input} /></div><div style={styles.formGroup}><label>Min Rate ($/hr)</label><input type="number" name="rateMin" value={formData.rateMin} onChange={handleChange} placeholder="Minimum" style={styles.input} /></div><div style={styles.formGroup}><label>Max Rate ($/hr)</label><input type="number" name="rateMax" value={formData.rateMax} onChange={handleChange} placeholder="Maximum" style={styles.input} /></div></div>
          <button type="submit" disabled={loading} style={styles.submitBtn}>{loading ? 'Posting...' : 'Post Job'}</button>
        </form>
      </div>
    </div>
  );
